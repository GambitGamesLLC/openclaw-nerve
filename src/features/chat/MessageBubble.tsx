...
function MessageBubbleInner({ msg, index, isCollapsed, isMemoryCollapsed, memoryKey, onToggleCollapse, onToggleMemory, firstMessageTime, searchQuery, isCurrentMatch, agentName, onOpenWorkspacePath, onOpenBeadId }: MessageBubbleProps) {
        ...
        {!isCollapsed && (
          <div className="ml-3 border-l border-primary/12 px-3 pb-2 pt-1 text-[0.8rem] text-foreground/70 msg-body-intermediate">
            <Suspense fallback={<span className="text-muted-foreground text-xs">…</span>}>
              <MarkdownRenderer content={msg.rawText} searchQuery={searchQuery} onOpenWorkspacePath={onOpenWorkspacePath} onOpenBeadId={onOpenBeadId} />
            </Suspense>
          </div>
        )}
      </div>
        ...
        >
          ...
          {isCollapsed ? (
            ...
            <div className="text-muted-foreground/70 text-[0.8rem] flex-1 min-w-0 msg-body-intermediate">
              <Suspense fallback={<span className="text-muted-foreground text-xs">…</span>}>
                <MarkdownRenderer content={displayContent} searchQuery={searchQuery} suppressImages={isAssistant} onOpenWorkspacePath={onOpenWorkspacePath} onOpenBeadId={onOpenBeadId} />
              </Suspense>
            </div>
          )}
